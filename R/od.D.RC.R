...
#
# Description:
# An implementation the algorithm for computing D-efficient designs from
#
...
#   If w1=NULL, use a random initial design.
...
#
# Works reasonably well for most problems with n<=200, m<=10, N<=50
...
info <- paste("Running D.RC for cca", t.max, "seconds")
...
while(!finish){ 
  ...
  if (tm > next.sec){
     info <- paste("Time:", round(tm, 1), "secs, Best value:", (det(t((w.best %*% tone.m) * F) %*% F))^(1/m))
     print(info, quote=FALSE)
     next.sec <- ceiling(tm)
  }
...
info <- paste("D.RC finished after", t.act, "seconds at", Sys.time())
print(info, quote=FALSE)
