od.SOCP <-
function(F, b, A=NULL, w0=NULL, crit="D", R=NULL, kappa=1e-9, tab=NULL, graph=NULL, t.max=120){
...
if (crit=="D") r<-od.D.SOCP(F=F, b=b, A=A, type="approximate", t.max=t.max, kappa=kappa)
if (crit=="A") r<-od.A.SOCP(F=F, b=b, A=A, type="approximate", t.max=t.max, kappa=kappa)
if (crit=="IV") r<-od.A.SOCP(F=F.IVtoA(F, R), b=b, A=A, type="approximate", t.max=t.max, kappa=kappa)

res<-list(method="Second-order Cone Programming", w.best=r$w.best, Phi.best=r$Phi.best, status=r$status, t.act=r$t.act)

if ((!is.null(graph)) && (r$Phi.best>1e-6)){
  title <- paste("od.SOCP: n=", n, ", m=", m, ", crit=", crit ,", Phi=", sep="")
